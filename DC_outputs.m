...
% %     MFII
    [Data_MFII, Data_MFII_1vR] = MFII(x,y,K_para);
% %     Class distributd metrics    包含三个 multi_IR  lrid_value  ID_HD  ID_TV
    IM_metric=ImbalancedMeasure(train_data);
    % 得到不同复杂性度量
    Outputs=[Data_F1(1),Data_F3,Data_N1,Data_N2,Data_N3,Data_LSC,Data_T1,Data_MFII];
    Avg_opt=[sum(avg.*Data_F1v_1vR),sum(avg.*Data_F3_1vR),sum(avg.*Data_N1_1vR),sum(avg.*Data_N2_1vR),sum(avg.*Data_N3_1vR),sum(avg.*Data_LSC_1vR),sum(avg.*Data_T1_1vR),sum(avg.*Data_MFII_1vR)];
    pos_opt=[sum(pos.*Data_F1v_1vR),sum(pos.*Data_F3_1vR),sum(pos.*Data_N1_1vR),sum(pos.*Data_N2_1vR),sum(pos.*Data_N3_1vR),sum(pos.*Data_LSC_1vR),sum(pos.*Data_T1_1vR),sum(pos.*Data_MFII_1vR)];
    neg_opt=[sum(neg.*Data_F1v_1vR),sum(neg.*Data_F3_1vR),sum(neg.*Data_N1_1vR),sum(neg.*Data_N2_1vR),sum(neg.*Data_N3_1vR),sum(neg.*Data_LSC_1vR),sum(neg.*Data_T1_1vR),sum(neg.*Data_MFII_1vR)];
    DC=[DC;Outputs]; 
